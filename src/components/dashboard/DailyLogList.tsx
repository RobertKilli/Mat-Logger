              ...
              {mealLogs.map((log) => (
                <div key={log.id} className="flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors group">
                  <div className="flex flex-col">
                    <span className="font-bold text-sm text-zinc-200">{log.name}</span>
                    <span className="font-mono text-[9px] text-zinc-500 uppercase">
                      {format(new Date(log.time), 'HH:mm')} • {log.inputAmount} {log.inputMode === 'GRAMS' ? 'g' : 'stk'} {log.inputMode === 'UNITS' && `(${log.weight}g)`}
                    </span>
                  </div>
